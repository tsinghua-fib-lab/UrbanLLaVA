...
echo "Start running mobility"
for MODEL in "${MODELS[@]}"; do
    echo "Current model: $MODEL"
    for CITY in "${CITIES[@]}"; do
        echo "Current city: $CITY"
        python -m evaluate.mobility_prediction.llm_mob --model_name $MODEL --data_name $DATA_VERSION --city_name $CITY 
    done
done
echo "Finish running mobility"
